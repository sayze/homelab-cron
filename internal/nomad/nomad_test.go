...
func TestHTTPClient_Version(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			respBody:    map[string]any{"config": map[string]any{"Version": map[string]any{"Version": "1.11.3"}}},
			wantVersion: "1.11.3",
		},
		{
			...
			wantErr:    "agent self response has no config.Version.Version",
		},
		{
			name:       "non-200 status",
...
func TestHTTPClient_Version_SendsToken(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		require.NoError(t, json.NewEncoder(w).Encode(map[string]any{"config": map[string]any{"Version": map[string]any{"Version": "1.11.3"}}}))
	}))
	defer srv.Close()
