...
type Config struct {
	...
	// bind mount (see homelab-cron.nomad.hcl: "/:/host:ro,rslave"); in
	// docker-compose it's a bind mount too. Jobs that need to observe host
	// state should read under this path (e.g.
	// filepath.Join(cfg.HostRoot, "var/log")) — this service only ever
	// reads the host, never writes to it.
	HostRoot string
